...
def optimizeWithIP(x0, A, b, C, d, mu=1.0, rate=0.1, tol=1e-6, max_iter=1000):
    ...
    # *************************************************************************
    #
    F[id_v] = v*s - mu # diff Langrangian w.r.t v 
    F[id_x] = A.T @(A @ x - b) + C.T @ v # diff Langrangian w.r.t x
    dF[id_x,id_x] = A.T @ A
    dF[id_x,id_v] = C.T
    ...
    while err >= tol:
        ...
        alpha = min(alpha,0.99*np.min(s[valid_x]/Cdx[valid_x]))
